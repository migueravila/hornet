...
fn main() {
    let matches = App::new("hornet")
        ...
        .subcommand(
            SubCommand::with_name("add")
                ...
                .arg(Arg::with_name("title").help("Todo title").required(true))
                .arg(
    ...
    if let Some(matches) = matches.subcommand_matches("add") {
        ...
        if let Some(project) = matches.value_of("project") {
            add_todo_to_project(title, project);
        } else {
            add_todo(title);
        }
...
fn fetch_today_todos() -> Vec<Todo> {
    ...
    if output.status.success() {
        ...
        println!("Debug - Raw output: {}", raw_output);
...
fn add_todo_to_project(title: &str, project_name: &str) {
    let script = format!(
        r#"
        tell application "Things3"
            try
